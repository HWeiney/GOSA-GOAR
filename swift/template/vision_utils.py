...
def _dynamic_preprocess(image,
    ...
    for i in range(blocks):
        ...
        tile_coords.append([col / grid_width, row / grid_height, (col + 1) / grid_width,
                            (row + 1) / grid_height])
        tile_types.append(0)
    ...
    if use_thumbnail and len(processed_images) != 1:
        ...
        tile_coords.append([0., 0., 1., 1.])
        tile_types.append(1)
    if return_tile_coords:
        return (processed_images, torch.tensor(tile_coords, dtype=torch.float32),
                torch.tensor(tile_types, dtype=torch.long))
...
def transform_image(image, input_size=448, max_num=12, return_tile_coords=False):
    ...
    images = _dynamic_preprocess(
        ...
        use_thumbnail=True,
        max_num=max_num,
        return_tile_coords=return_tile_coords)
    if return_tile_coords:
        images, tile_coords, tile_types = images
    pixel_values = [transform(image) for image in images]
    pixel_values = torch.stack(pixel_values)
    if return_tile_coords:
        return pixel_values, tile_coords, tile_types
    return pixel_values
