...
def _patch_qwen3_5_linear_attention_sequence_parallel() -> None:
    ...
    origin_forward_params = inspect.signature(origin_forward).parameters
    ...
    def sp_linear_forward(
        ...
        if not _sp_is_enabled(sequence_parallel_context):
            ...
            # Qwen3.5 changed this forward signature across transformers
            # releases. In particular, 5.5 no longer accepts cache_position
            # on GatedDeltaNet. Only forward arguments supported by the
            # installed transformers version should reach the original layer.
            origin_kwargs = {key: value for key, value in origin_kwargs.items() if key in origin_forward_params}
            return origin_forward(mod, hidden_states, **origin_kwargs)
