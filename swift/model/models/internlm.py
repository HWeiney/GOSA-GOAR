...
class InternVLLoader(ModelLoader):
    ...
    def get_model(self, model_dir: str, *args, **kwargs) -> PreTrainedModel:
        ...
        if model is not None:
            from ..internvl_extensions import attach_gosa
            attach_gosa(model, model_dir)
        ...
        if model is not None:
            # GOAR wraps the delegated language-model forward, so attach it
            # only after ``use_submodel_func`` has installed that delegation.
            from ..internvl_extensions import attach_goar
            attach_goar(model)
        patch_output_clone(model.language_model.get_input_embeddings())
        ocr_mode = os.environ.get('OCR_MODE', 'none').strip().lower()
        if model is not None and ocr_mode == 'lilt_qformer':
            from ..ocr_evidence import OCREvidenceEncoder, load_ocr_encoder_state
            lilt_path = os.environ.get(
                'LILT_MODEL_PATH',
                os.path.join(os.path.dirname(model_dir), 'lilt-infoxlm-base'))
            ...
            if llm_hidden_size is None:
                raise ValueError('Cannot determine the InternVL LLM hidden size from model.config.llm_config.')
            model.ocr_evidence_encoder = OCREvidenceEncoder(
                lilt_path=lilt_path,
                llm_hidden_size=llm_hidden_size,
                num_queries=int(os.environ.get('OCR_NUM_QUERIES', '64')),
                max_ocr_length=int(os.environ.get('OCR_MAX_LENGTH', '512')),
                unfreeze_lilt_top_layers=int(os.environ.get('OCR_UNFREEZE_LILT_TOP_LAYERS', '0')),
            )
            load_ocr_encoder_state(model.ocr_evidence_encoder, model_dir)
            model.config.ocr_mode = ocr_mode
            model.config.lilt_model_path = lilt_path
            model.config.ocr_num_queries = model.ocr_evidence_encoder.num_queries
        return model
