...
class DocOwl2Loader(ModelLoader):
    ...
    def _get_model_processor(self, model_dir, config):
        model, tokenizer = super()._get_model_processor(model_dir, config)
        if model:
            # DocOwl2 bundles a Transformers 4.31-era Llama implementation
            # that reads and returns tuple-based KV caches. Transformers 4.57
            # otherwise preallocates DynamicCache, which the bundled attention
            # layers do not populate and generation fails on the second token.
