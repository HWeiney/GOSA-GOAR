...
class Seq2SeqTrainer(SwiftMixin, DataLoaderMixin, HfSeq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Optional structural losses produced by multimodal extensions such as
        # InternVL GOAR. Keeping this after the ordinary LM-loss paths makes it
        # compatible with token weighting, DFT, and label smoothing.
        goar_state_host = self.accelerator.unwrap_model(model)
        goar_loss = getattr(goar_state_host, '_goar_aux_loss', None)
        goar_applied = getattr(goar_state_host, '_goar_forward_applied', False)
        goar_state_host._goar_aux_loss = None
        goar_state_host._goar_forward_applied = False
        if (model.training and os.environ.get('ENABLE_GOAR', 'False').lower() == 'true'
                and not goar_applied):
            raise RuntimeError(
                'ENABLE_GOAR=True but GOAR was not applied in the model forward; refusing silent baseline training')
        ...
        if goar_loss is not None:
            # The LM loss above is normalized with the token count accumulated
            # across micro-batches and then corrected for DDP's gradient
            # averaging. GOAR is already a local per-sample mean, so it must not
            # receive the world-size multiplier and must be averaged explicitly
            # over gradient-accumulation micro-batches.
            if model.training:
                goar_loss = goar_loss / self.current_gradient_accumulation_steps
            loss = loss + goar_loss.to(loss.device)
    ...
    def training_step(self, model, inputs, *args, **kwargs):
        ...
        if os.environ.get('ENABLE_GOAR', 'False').lower() == 'true' and not hasattr(self, '_goar_grad_audited'):
            unwrapped = self.accelerator.unwrap_model(model)
            params = [(name, parameter) for name, parameter in unwrapped.named_parameters()
                      if 'goar_adapter' in name and parameter.requires_grad]
            with_grad = [(name, parameter) for name, parameter in params if parameter.grad is not None]
            nonzero = [(name, parameter) for name, parameter in with_grad if parameter.grad.detach().abs().max() > 0]
            print(f'GOAR gradient audit: trainable={len(params)}, with_grad={len(with_grad)}, nonzero={len(nonzero)}')
            self._goar_grad_audited = True
            if not nonzero:
                raise RuntimeError('GOAR has no nonzero gradients after backward; refusing structurally inactive training')
        return loss
